...
create_dns() {
	...
	domain_id=`cat $1 | jq -r '.domains[] | select(.name | inside("'$TARGET_DOMAIN'.")) | .id'`
	if [ -z "$domain_id" ]; then
	...
	curl $DNS_SERVICE/v1/domains/$domain_id/records -X POST -H "Accept: application/json" -H "Content-Type: application/json" -H "X-Auth-Token: $token_id" \
		-d '{ "name": "'$TARGET_NAME'.'$TARGET_DOMAIN'.", "type": "'$TARGET_TYPE'", "data": "'$TARGET_DATA'", "ttl": 60 }' > $1 2> /dev/null
	if [ $? -ne 0 ]; then
